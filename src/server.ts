...
import { CustomError, IErrorResponse, winstonLogger } from '@chandrashekar2001/ecommerce-shared';
...
export class GatewayServer {
  ...
  private errorHandler(app: Application): void {
    app.use('*', (req: Request, res: Response, next: NextFunction) => {
      ...
      log.log('error', `${fullUrl} URL does not exist.`, '');
      res.status(StatusCodes.NOT_FOUND).json({ message: 'The requested url called does not exist.'});
      next();
    });

    app.use((error: IErrorResponse, _req: Request, res: Response, next: NextFunction) => {
      if (error instanceof CustomError) {
        log.log('error', `GatewayService :`, error);
